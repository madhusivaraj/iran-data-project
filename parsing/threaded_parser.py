...
#############################
# Multi-threading beyond this point
#############################
import queue
...
def async_parser(file_name):
    global parsedCount, file_queue, error_queue, results_queue
    ...
    try:
        if DEBUG:
            ...
            print('-----------------------')
        data = parse(file_name) # Grab all the data we support extracting
        if DEBUG:
            print(data)
        if not data == None:
            parsedCount += 1
        ...
        file_queue.get() # Show we are done
        print('Worker finished for', file_name)
    except:
        print('Worker died for', file_name)
...
def parse_folder(folder):
    global file_queue, error_queue, results_queue, parsedCount
    ...
    try:
        ...
        to_parse_full = []
        for i in files:
            to_parse_full += [ folder + '/' + i ]
    except:
        raise Exception('I just can\'t even')
        pass
    parsedCount = 0
    batch = 0
    for to_parse in chunks(to_parse_full, 3000):
        batch += 1
        while len(to_parse) > 0:
            if file_queue.qsize() < BATCH_SIZE:
                new_file = to_parse.pop()
                print('Spawning worker for ', new_file)
                t1 = Thread(target=async_parser, args=[new_file])
                t1.start()
                file_queue.put(new_file) # Could put in anything
            if error_queue.qsize() > 0:
                raise Exception("Error on a thread")

        while file_queue.qsize() > 0:
            pass # Ensure all workers are finished

        all_data = []
        while not results_queue.empty():
            all_data += [results_queue.get()]
        '''
        for i in to_parse:
            if DEBUG:
                print()
